...
flota. **Ciclo (de facturación)** = periodo mensual del contrato anclado al aniversario de su
`fecha_inicio` con clamp de fin de mes (inicio 20-ago → ciclo 20-ago…19-sep; D17), unidad de
cobro que conserva su `tarifa_base` y `paginas_incluidas`. **Lectura de corte** = última
lectura no facturada del contrato dentro de `[fin del ciclo − 5, fin + dias_gracia]`; cierra
el ciclo para facturación y activa el arrastre de consumo (D22).
...
| D17 | **Estado de facturación por contrato con ciclos mensuales por aniversario de `fecha_inicio`** | El socio necesita saber qué ciclos cobró por contrato, no solo qué lecturas facturó; el ancla del periodo es el aniversario de `fecha_inicio` (ej. inicio 20-ago → ciclos 20→19), no el mes calendario. Ciclo N = `[fecha_inicio + N meses (clamp fin de mes con `addMonthsNoOverflow`), inicio del siguiente − 1 día]`, recortado a la vigencia (`fecha_inicio`/`fecha_fin`); los inicios SIEMPRE se derivan de `fecha_inicio` (nunca encadenados del ciclo anterior) para que los días 29/30/31 recuperen su día. Cobertura conservadora por intersección de rangos (una factura que toca el contrato cubre todo ciclo que intersecta, incluye borradores). Clave de periodo `AAAA-MM-DD` (fecha de inicio del ciclo). Decisiones cerradas: la lectura del día de inicio pertenece al ciclo que abre; clamp de fin de mes para días 29/30/31; batch siempre por contrato; el flujo manual a nivel cliente captura rango libre (sin picker de mes) | `CicloFacturacion` (Support) es la única fuente de la aritmética de ciclos; `ContractBillingService`/`GET /contracts/{id}/facturacion` es la fuente de ciclos facturados vs pendientes. Fuera de alcance a propósito: reportes/caja/rentabilidad siguen bucketizando por mes calendario contable (`periodo_inicio`), y no hay prorrateo de tarifa en ciclos parciales. Nota D22: una factura de ciclo puede incluir lecturas con `fecha < periodo_inicio` (la lectura mide el hueco acumulado; el periodo facturado sigue siendo el ciclo) |
| D18 | **Batch de borradores: un borrador por ciclo, nunca fusionados** | Cada ciclo debe conservar su `tarifa_base` y `paginas_incluidas`; fusionar ciclos en una factura subcobra renta | La estimación advierte (no bloquea) cuando un rango manual cubre >~1.5 meses: tarifa única por factura (un ciclo ≈ 1 mes, no da falso positivo) |
...
| D23 | **Retiro por `SUSTITUCION_FALLA` puede crear la orden correctiva en la misma transacción (flag explícito)** | El operador ya está frente a la impresora fallando: evita re-capturar el problema en el panel web. El flag (`crear_orden_mantenimiento`) es explícito; con ese motivo la orden nace CORRECTIVA y `desc_problema` es requerida (D24 generaliza el flag al resto de motivos). La orden nace vía `MaintenanceService::create` dentro de la transacción del retiro, después de liberar la impresora | La impresora queda EN_MANTENIMIENTO con `estado_anterior=EN_ALMACEN`; EN_MANTENIMIENTO **sigue asignada al contrato y facturando renta** (decisión documentada); la historia de liberación guarda `orden_mantto_id` en `datos_adicionales`; `finish()`/`cancel()` de contrato nunca crean órdenes (defaults del parámetro) |
| D24 | **Retiro con orden de servicio para cualquier motivo + bloqueo duro por orden abierta** | Unifica web y móvil: retirar con `crear_orden_mantenimiento=true` crea la orden en la misma transacción para **cualquier** motivo (`SUSTITUCION_FALLA` ⇒ CORRECTIVA con `desc_problema` requerida; cualquier otro ⇒ PREVENTIVA con notas opcionales, autocompletadas como "Servicio preventivo al retirar del contrato {codigo_negocio}"). El tipo lo deriva el backend del motivo (el cliente nunca lo envía) y exige además `inventario.mantenimiento` (403 si falta) | Cero migraciones ni estado nuevo: se reusa `EN_MANTENIMIENTO` + `estado_anterior_impresora` (D1 descartó `EN_REVISION`). `MaintenanceService::create` gana flag explícito `sacarDeCirculacion` (las preventivas web sin flag NO tocan el estado); `restorePrinterState` se keyed por `estado_anterior_impresora` (no por tipo), escribiendo `MANTENIMIENTO_PREVENTIVO_FIN` al completar (además del evento `MANTENIMIENTO_PREVENTIVO` que se mantiene siempre); guard anti-duplicado (422 si ya hay orden PROGRAMADA) y `assignPrinter` bloquea 422 mientras exista una orden PROGRAMADA; el catálogo web/móvil expone `ordenes_abiertas_count` + `open_maintenance_order` (chip informativo, no filtro) |
| D25 | **La orden nace con `fecha` = reporte (auto-estampada); "programar" = el área técnica edita la Fecha objetivo de la orden PROGRAMADA; sin estado PENDIENTE ni visitas automáticas desde órdenes (el scheduler D2/D8 queda intacto). Ubicación taller/piso derivada de cliente/warehouse, mostrada en detalle y cola** | Levantar una orden es una solicitud: el operador/admin reporta, pero no dispone del tiempo del técnico; programar es decisión del área técnica (cola por severidad + antigüedad). La fecha==reporte equivale a "sin programar", por lo que un estado PENDIENTE previo a PROGRAMADA sería redundante | El alta web ya no envía `fecha` ni `costo_mano_obra` (el `StoreMaintenanceOrderRequest` auto-estampa `today()` si falta y el costo se captura solo al completar; el móvil sigue enviando fecha explícita — compatible). El detalle y la cola del Taller muestran "Fecha objetivo" con chip de objetivo vencido (`fecha < hoy` en PROGRAMADA); `MaintenanceService::update()` invalida `taller.dashboard`. La ubicación se deriva: `currentAssignment.contract.client` ⇒ "En piso", si no `warehouse` ⇒ "En taller" (el detalle añade la próxima visita pendiente del contrato como contexto) |
